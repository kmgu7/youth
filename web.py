...
def home():
    ...
    grouped = defaultdict(list)
    for idx, article in enumerate(ARTICLES):
        cat = article.get("세부카테고리", "기타")
        if len(grouped[cat]) < 2:  # 카테고리당 2개만
            grouped[cat].append((idx, article))
    
    # 대표 카테고리 6개만 추출 (있는 순서대로)
    selected_cats = list(grouped.keys())[:6]
    filtered = {cat: grouped[cat] for cat in selected_cats}
    
    return render_template("home.html",
                           subcategories=SUBCATEGORIES,
                           selected_category="홈",
                           home_articles=filtered)
